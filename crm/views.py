            ...
            for index, stop in enumerate(stops, start=1):
                ...
                story.extend([
                    ...
                    field_table([
                        ...
                        ("Адрес", f"{stop.city or '—'} · {stop.full_address or '—'}"),
...
class TransportationListView(LoginRequiredMixin, PersistentPageSizeMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if trip_date_from:
            queryset = queryset.filter(
                **{f"{trip_date_fields[trip_date_type]}__gte": trip_date_from}
            )
        if trip_date_to:
            queryset = queryset.filter(
                **{f"{trip_date_fields[trip_date_type]}__lte": trip_date_to}
            )
...
class DriverDetailView(PersonalDataViewMixin, LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        transportation_assignments = list(
            ...
            .prefetch_related("transportation__stops", "transportation__parties__organization")
        )
